    ...
    for i, row in enumerate(seatmap):
        ...
        for j, char in enumerate(row):
            ...
            if char==".":
                continue
            if char=="L":
                occ_seats=see(old_grid,seatmap,i,j,-1,-1)+see(old_grid,seatmap,i,j,-1,0)+see(old_grid,seatmap,i,j,-1,+1)+see(old_grid,seatmap,i,j,0,-1)+see(old_grid,seatmap,i,j,0,+1)+see(old_grid,seatmap,i,j,+1,-1)+see(old_grid,seatmap,i,j,+1,0)+see(old_grid,seatmap,i,j,+1,+1)
                if occ_seats == 0:
                    grid[i,j]=1
                if occ_seats >= 5:
                    if grid[i,j]==1:
